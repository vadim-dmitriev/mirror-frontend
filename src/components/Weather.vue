<template>
	<div class="weather">
		...
		<div class="weather-bottom">
			<div>
				Ветер {{ currentWindSpeedViewed }}м/с
			</div>
			<div>
				Влажность {{ currentHumidityViewed }}%
			</div>
		</div>
		...
		<div class="weather-forecast">
			<div class="weather-forecast-day" v-for="item in forecast" :key="item">
				...
				<div class="weather-forecast-day-main">
					<div>
						<img class="weather-forecast-day-icon" v-bind:src="weatherForecastIcon(item.kind)" />
					</div>
					<div>
						{{ tempratureViewed(item.minTemp) }}...{{ tempratureViewed(item.maxTemp) }}
					</div>
				</div>
				<div class="weather-forecast-day-precipitation">
					<div>
						<img style="height:20px;opacity:0.6" src="../assets/icons/weather/precipitation.png" />
					</div>
					<div>
						{{ item.pop }}%
					</div>
...
export default {
	...
	methods: {
		updateWeatherData: async function () {
			...
			for (let i = 0; i < this.forecast.length; i++) {
				this.forecast[i].dayOfWeek = days[(currentTime.getDay() + i) % days.length];
			}
		},
	...
	computed: {
		currentWeatherKindIcon: function () {
			...
			return require("@/assets/icons/weather/" + this.currentWeatherKind + ".png")
		},
